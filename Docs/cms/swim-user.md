- 🏊‍♀️ 수영장 **신청 & 신청내역 확인** — 사용자-측 개발문서
  ...
  ***
  ...
  | POST   | /api/v1/swimming/enroll               | EnrollRequestDto             | EnrollResponseDto      | 수업 신청 및 결제                 |
  ...
  ***
  ...
  CREATE TABLE enroll (
      ...
      status VARCHAR(20) NOT NULL COMMENT '신청 상태(APPLIED, CANCELED, PENDING)',
      pay_status VARCHAR(20) NOT NULL DEFAULT 'UNPAID' COMMENT '결제 상태(UNPAID, PAID, CANCELED_UNPAID)',
      ...
      UNIQUE KEY uk_user_lesson_paid (user_id, lesson_id, pay_status) COMMENT '사용자별 강습 중복 신청 방지',
  ...
  ) COMMENT '강습 신청 정보 테이블';

  -- 결제 테이블: 결제 정보를 저장하는 테이블
  CREATE TABLE payment (
      payment_id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '결제 ID (PK)',
      enroll_id BIGINT NOT NULL COMMENT '신청 ID (FK)',
