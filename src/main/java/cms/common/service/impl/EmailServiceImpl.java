...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendUserIdEmail(String toEmail, String userId, String name) throws EmailSendingException {
        ...
        try {
            ...
            log.debug("Generated HTML body for User ID email (first 100 chars): {}", htmlBody.substring(0, Math.min(htmlBody.length(), 100)));
        } catch (Exception e) {
            log.error("Error processing Thymeleaf template 'email/find-id-notification' for {}: {}", toEmail, e.getMessage(), e);
        ...
        if (htmlBody == null || htmlBody.trim().isEmpty()) {
            log.warn("Generated HTML body for User ID email is empty for {}. Check template 'email/find-id-notification'.", toEmail);
            ...
            // sendEmail(toEmail, subject, htmlBody, false); // false는 HTML이 아님을 의미 (sendEmail 메소드 수정 필요)
        }
    ...
    @Async
    @Override
    public void sendTemporaryPasswordEmail(String toEmail, String temporaryPassword, String name) throws EmailSendingException {
    ...
    private void sendEmail(String to, String subject, String htmlBody) throws EmailSendingException {
        ...
        try {
            ...
            log.debug("Attempting to set recipient email for '{}': '[{}]', length: {}", subject, to, to != null ? to.length() : "null");
            helper.setTo(to);
            ...
            log.error("Failed to send email to '{}' due to MessagingException. Subject: {}. Error: {}", to, subject, e.getMessage(), e);
            ...
            log.error("An unexpected error occurred while sending email to '{}'. Subject: {}. Error: {}", to, subject, e.getMessage(), e);
            throw new EmailSendingException("메일 발송 중 예상치 못한 오류가 발생했습니다: " + e.getMessage(), "UNEXPECTED_EMAIL_ERROR", e);
        }
    }
} 