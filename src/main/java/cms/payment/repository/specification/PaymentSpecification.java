...
public class PaymentSpecification {
    ...
    public static Specification<Payment> filterByAdminCriteria(
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            if (enrollId != null) {
                if (enrollJoin == null) enrollJoin = root.join("enroll");
                predicates.add(criteriaBuilder.equal(enrollJoin.get("enrollId"), enrollId));
            }

            if (userId != null && !userId.trim().isEmpty()) {
                if (enrollJoin == null) enrollJoin = root.join("enroll");
                if (userJoin == null) userJoin = enrollJoin.join("user"); // Assign userJoin
            ...
            if (startDate != null) {
                predicates.add(criteriaBuilder.greaterThanOrEqualTo(root.get("paidAt"), LocalDateTime.of(startDate, LocalTime.MIN)));
            }

            if (endDate != null) {
                predicates.add(criteriaBuilder.lessThanOrEqualTo(root.get("paidAt"), LocalDateTime.of(endDate, LocalTime.MAX)));
            }
