...
public class PaymentController {
    ...
    @Operation(summary = "KISPG 결제 완료 후 리턴 URL", 
               description = "KISPG 인증 완료 후 자동으로 승인 요청을 처리합니다.")
    public ResponseEntity<ApiResponseSchema<String>> handleKispgReturn(
        ...
        try {
            ...
            if ("0000".equals(resultCd)) {
                ...
                kispgResultDto.setEdiDate(request.getParameter("ediDate")); // ediDate might not be a direct @RequestParam
                ...
                EnrollDto enrollDto = kispgPaymentService.approvePaymentAndCreateEnrollment(approvalRequest, currentUser);
    ...
    @Operation(summary = "KISPG 결제 승인 및 수강 등록",
               description = "프론트엔드에서 KISPG 결제 인증 완료 후, 실제 승인 및 수강 등록을 처리합니다.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<ApiResponseSchema<EnrollDto>> approveAndCreateEnrollment(
        ...
        try {
            ...
            return ResponseEntity.status(e.getErrorCode().getHttpStatus())
                                 .body(ApiResponseSchema.<EnrollDto>error(e.getErrorCode().getDefaultMessage(), e.getErrorCode().getCode()));
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                 .body(ApiResponseSchema.<EnrollDto>error(ErrorCode.PAYMENT_FAILED.getDefaultMessage(), ErrorCode.PAYMENT_FAILED.getCode()));
        }
    }
