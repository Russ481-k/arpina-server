...
public class BbsArticleController {
    ...
    @Operation(summary = "게시글 목록 및 검색 조회", description = "게시글 목록을 조회하거나, 키워드가 제공되면 검색 결과를 조회합니다.")
    @GetMapping
    public ResponseEntity<ApiResponseSchema<Page<BbsArticleDto>>> getArticles(
        ...
        @Parameter(description = "메뉴 ID") @RequestParam Long menuId,
        @Parameter(description = "검색 키워드 (제목, 내용, 작성자, 날짜[YYYY-MM-DD] 대상)") @RequestParam(required = false) String keyword,
        @Parameter(description = "페이지네이션 정보 (page, size, sort)") Pageable pageable) {
        
        Page<BbsArticleDto> articles;
        String message;

        if (keyword != null && !keyword.trim().isEmpty()) {
            articles = bbsArticleService.searchArticles(bbsId, menuId, keyword, pageable);
            message = String.format("키워드 '%s'에 대한 게시글 검색 결과를 성공적으로 조회했습니다.", keyword);
        } else {
            articles = bbsArticleService.getArticles(bbsId, menuId, pageable);
            message = "게시글 목록을 성공적으로 조회했습니다.";
        }
        return ResponseEntity.ok(ApiResponseSchema.success(articles, message));
    }
    ...
    public ResponseEntity<ApiResponseSchema<Long>> createArticle(
        ...
        BbsArticleDto createdArticle = bbsArticleService.createArticle(articleDto, editorContentJson, mediaFiles, mediaLocalIds, attachments);
    ...
    public ResponseEntity<ApiResponseSchema<Void>> updateArticle(
        ...
        return ResponseEntity.ok(ApiResponseSchema.success("게시글이 성공적으로 수정되었습니다."));
    }
